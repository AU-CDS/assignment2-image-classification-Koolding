...
def mlp_clf(X_train_dataset, y_train):
    clf = MLPClassifier(random_state=42,
                        hidden_layer_sizes=(64, 10), # defining the two hidden layers
                        learning_rate="adaptive",
                        early_stopping=True, # terminate training when validation score is not improving
                        verbose=True, # print progress messages
                        max_iter=20).fit(X_train_dataset, y_train) # number of iterations / amount of time the weights are updated. 
...
def clf_report(X_test_dataset, y_test, clf):
    ...
    report_path = os.path.join("reports", "mlpclf_report.txt")
